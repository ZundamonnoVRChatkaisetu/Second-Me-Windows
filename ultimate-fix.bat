...
:: Export environment variables for Node.js proxy
...
:: 4. Configure backend connection via CORS
echo [3/9] Setting up backend connection via CORS proxy...
...
:: 5. Create/update CORS proxy script
echo [4/9] Creating/updating CORS proxy script...
cd lpm_frontend
...
:: Remove existing CORS proxy script and create a new one
...
echo }); >> public\cors-anywhere.js
echo. >> public\cors-anywhere.js
echo // Add CORS headers function >> public\cors-anywhere.js
echo app.use((req, res, next) =^> { >> public\cors-anywhere.js
...
echo [6/9] Waiting for backend to initialize...
timeout /t 8 /nobreak > nul

:: 7. Start CORS proxy
echo [7/9] Starting CORS proxy...
cd lpm_frontend
...
:: Wait for CORS proxy to initialize
echo Waiting for CORS proxy to initialize...
...
echo [9/9] Starting browser...
timeout /t 8 /nobreak > nul
start http://localhost:%FRONTEND_PORT%

echo.
echo  ================================================
echo    All services fixed and started!
echo  ================================================
echo.
...
echo  1. Stop all services: taskkill /f /fi "WINDOWTITLE eq Second-Me*"
echo  2. Clean up frontend dependencies: lpm_frontend\clean-all.bat
echo  3. Change to frontend directory: cd lpm_frontend
echo  4. Install dependencies manually: npm install
echo  5. Run this script again after cleanup: cd .. && ultimate-fix.bat
echo.
